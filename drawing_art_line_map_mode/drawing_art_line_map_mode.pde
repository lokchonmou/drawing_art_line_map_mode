...
void setup() {
  ...
  text("Please Press the keyboard to Select Working Mode:\n 1. Live Webcam Mode   2. Photo Mode    3. Machine setting", width/2, height/2);
  text("Pen UP = "+ pen_up+"       Pen DOWN = "+pen_down, width/2, 50);
...
void draw() {
  if (selecting == false && preparing == true) {
    ...
    if (working_mode ==1) {
      ...
      frame.setSize(975, int(975*_scale));  //100dpi=3.9dpmm, 250mm*3.9dpmm= 975pixel 

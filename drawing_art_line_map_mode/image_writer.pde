...
void printer_output() {
  ...
  for (int i = 0; i<=2; i++) {
    ...
    for (Contour contour : contours) {
      if (contour.area() >10) {
        for (PVector point : contour.getPoints()) {
          point.x = point.x /3.94 + 10; //scale 300dpi
          point.y = point.y /3.94 + 10; //scale 300dpi
          output.println("G0 " + "X"+ point.x + " Y" +point.y + " Z0" + " F1000"); // Write the coordinate to the file
          if (!posMoved) {
            output.println("M1 "+ pen_down);
            posMoved = true;
          }
        }
        output.println("M1 "+ pen_up);
        posMoved = false;
      }
    }
  }
...
void to_serial_output() {
  ...
  if (oked) {
    println(float(forLoop)/float(gcodes.length)*100, '%', ' ', ':', gcodes[forLoop]);
...
void serialEvent(Serial p) {
  if (p.available()>0) {
    ...
    if (myString != null) {
      ...
      if (myString.equals("OK") && forLoop <= gcodes.length) {
        println(myString); 
        oked = true;
        forLoop++;
      }
